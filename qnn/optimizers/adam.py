    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                ...
                # Learning rate scaling tensor
                if hasattr(p, 'lr_scale'):
                    # If the parameter has a learning rate scaling tensor, use it
                    if p.lr_scale is not None:
                        lr_list.append(p.lr_scale)
                else:
                    lr_list.append(torch.ones_like(p))
...
def _single_tensor_adam(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        # Scale the base lr with the weight-specific learning rate from the dictionary (from the freeze agent)
    ...
    *,
    ...
    for (
        ...
        if capturable:
            ...
            # Scale the average gradient by the learning rate scaling tensor
            if lr_list is not None:
                torch._foreach_mul_(device_exp_avgs, lr_list)
...
def adam(
    ...
    lr_list: Optional[List[Tensor]] = None,
    *,
    amsgrad: bool,
    beta1: float,
    beta2: float,
